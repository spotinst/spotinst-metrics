 ...
 */
public class UpdateDummyCmd extends BaseCmd<Boolean> {
    private static final Logger LOGGER = LoggerFactory.getLogger(UpdateDummyCmd.class);

    public UpdateDummyCmd() {
        super(null, null);
    }

    public void execute(String dummyId, BlDummy blDummy) {
        LOGGER.info(String.format("Start updating dummy [%s]...", dummyId));

        blDummy.setId(dummyId);

        RepoGenericResponse<Boolean> response = SampleRepoManager.Dummy.update(dummyId, blDummy);
        // We can use the generic repository as well
        // RepoGenericResponse<Boolean> response = SampleRepoManager.DummyRepo.update(dummyId, blDummy);
        if (response.isRequestSucceed() == false) {
            String         errFormat = "Failed updating dummy [%s]";
            String         errMsg    = String.format(errFormat, dummyId);
            List<DalError> dalErrors = response.getDalErrors();
            LOGGER.error(errMsg, dalErrors);
            throw new BlException(ErrorCodes.FAILED_TO_UPDATE_DUMMY, "Failed to update dummy", dalErrors);
        }

        LOGGER.info(String.format("Finished updating dummy [%s]", dummyId));
