...
public class MetricRangeAggregationParser extends BaseMetricAggregationParser<ElasticMetricStatisticsRequest> {
    ...
    public Map<AggCompositeKey, Map<String, Aggregate>> parse(
        ...
        for (AggCompositeKey compositeKey : aggsMapByKeys.keySet()) {
            ...
            List<RangeBucket> buckets = rangeAggregate.buckets().array();

            if (CollectionUtils.isNotEmpty(buckets)) {
                for (RangeBucket bucket : buckets) {
                    String rangeValue = bucket.key();

                    // Create the range dimension
                    ElasticMetricDimension rangedDimension =
                            new ElasticMetricDimension(metricStatisticsRequest.getMetricName(), rangeValue);
                    ...
                    List<ElasticMetricDatapoint> deepCopyDatapoints =
                            deepCopyDatapoints(iterationMetricSequence.getDatapoints());
                    ...
                    List<ElasticMetricDimension> dimensionItr = iterationMetricSequence.getDimensions();
                    List<ElasticMetricDimension> newDims      = new LinkedList<>();

                    if (CollectionUtils.isNotEmpty(dimensionItr)) {
                        dimensionItr.forEach(d -> {
